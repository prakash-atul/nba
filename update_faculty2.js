const fs = require("fs");
const content = fs.readFileSync("api/controllers/FacultyController.php", "utf8");

const newFunc = `    public function getEnrolledStudents($facultyId)
    {
        try {
            ...
            if (!empty($students)) {
                ...
                if ($rawSort !== "roll_no") {
                    $nextCursor = $lastItem[$rawSort] . "|" . $lastItem["roll_no"];
                } else {
...
const replacedRegex = /public function getEnrolledStudents\(\$facultyId\)[^\{]*\{[\s\S]*?\n    \}\r?\n\r?\n    \/\*\*/;
if (replacedRegex.test(content)) {
    const updated = content.replace(replacedRegex, newFunc + "\n\n    /**");
    fs.writeFileSync("api/controllers/FacultyController.php", updated);
